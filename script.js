  ...
  const profileAvatarEl = document.querySelector('.profile-avatar');
    ...
    // Update avatar initial and bg for demo user or generic users
    if (storedEmail === DEMO_USER.email) {
      if (profileInitialEl) profileInitialEl.textContent = 'D';
      if (profileAvatarEl) profileAvatarEl.style.background = '#f59e0b'; // orange
    } else {
      const initial = (storedName?.trim()?.charAt(0) || storedEmail?.trim()?.charAt(0) || 'U').toUpperCase();
      if (profileInitialEl) profileInitialEl.textContent = initial;
      if (profileAvatarEl) profileAvatarEl.style.background = '';
    }
    } else {
    ...
    if (profileInitialEl) profileInitialEl.textContent = 'U';
    if (profileAvatarEl) profileAvatarEl.style.background = '';
    }
  }
